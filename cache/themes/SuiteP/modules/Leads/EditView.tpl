...
{minify}
{capture name="label" assign="label"}{sugar_translate label='LBL_EVENT_NAME' module='Leads'}{/capture}
...
{minify}
{capture name="label" assign="label"}{sugar_translate label='LBL_CHARITY_NAME' module='Leads'}{/capture}
{$label|strip_semicolon}:
...
{minify}
{capture name="label" assign="label"}{sugar_translate label='LBL_EVENT_TYPE' module='Leads'}{/capture}
{$label|strip_semicolon}:
...
{if !isset($config.enable_autocomplete) || $config.enable_autocomplete==false}
<select name="{$fields.event_type_c.name}" 
id="{$fields.event_type_c.name}" 
title=''       
>
{if isset($fields.event_type_c.value) && $fields.event_type_c.value != ''}
{html_options options=$fields.event_type_c.options selected=$fields.event_type_c.value}
{else}
{html_options options=$fields.event_type_c.options selected=$fields.event_type_c.default}
{/if}
</select>
{else}
{assign var="field_options" value=$fields.event_type_c.options }
{capture name="field_val"}{$fields.event_type_c.value}{/capture}
{assign var="field_val" value=$smarty.capture.field_val}
{capture name="ac_key"}{$fields.event_type_c.name}{/capture}
{assign var="ac_key" value=$smarty.capture.ac_key}
<select style='display:none' name="{$fields.event_type_c.name}" 
id="{$fields.event_type_c.name}" 
title=''          
>
{if isset($fields.event_type_c.value) && $fields.event_type_c.value != ''}
...
<input
id="{$fields.event_type_c.name}-input"
name="{$fields.event_type_c.name}-input"
size="30"
value="{$field_val|lookup:$field_options}"
type="text" style="vertical-align: top;">
<span class="id-ff multiple">
<button type="button"><img src="{sugar_getimagepath file="id-ff-down.png"}" id="{$fields.event_type_c.name}-image"></button><button type="button"
id="btn-clear-{$fields.event_type_c.name}-input"
title="Clear"
onclick="SUGAR.clearRelateField(this.form, '{$fields.event_type_c.name}-input', '{$fields.event_type_c.name}');sync_{$fields.event_type_c.name}()"><span class="suitepicon suitepicon-action-clear"></span></button>
</span>
{literal}
<script>
	SUGAR.AutoComplete.{/literal}{$ac_key}{literal} = [];
	{/literal}
			...
			{literal}
		(function (){
			var selectElem = document.getElementById("{/literal}{$fields.event_type_c.name}{literal}");
	...
	SUGAR.AutoComplete.{$ac_key}.inputNode = Y.one('#{$fields.event_type_c.name}-input');
	SUGAR.AutoComplete.{$ac_key}.inputImage = Y.one('#{$fields.event_type_c.name}-image');
	SUGAR.AutoComplete.{$ac_key}.inputHidden = Y.one('#{$fields.event_type_c.name}');
			...
			//global variable 
			sync_{/literal}{$fields.event_type_c.name}{literal} = function(){
				SyncToHidden();
			}
			function syncFromHiddenToWidget(){
				...
				for (i=0;i<selectElem.options.length;i++){
					...
					if (selectElem.options[i].value==selectElem.value && document.activeElement != document.getElementById('{/literal}{$fields.event_type_c.name}-input{literal}'))
						SUGAR.AutoComplete.{/literal}{$ac_key}{literal}.inputNode.set('value',selectElem.options[i].innerHTML);
				}
			}

            YAHOO.util.Event.onAvailable("{/literal}{$fields.event_type_c.name}{literal}", syncFromHiddenToWidget);
		{/literal}
...
{minify}
{capture name="label" assign="label"}{sugar_translate label='LBL_DATE_SENT' module='Leads'}{/capture}
{$label|strip_semicolon}:
...
<span class="dateTime">
{assign var=date_value value=$fields.date_sent_c.value }
<input class="date_input" autocomplete="off" type="text" name="{$fields.date_sent_c.name}" id="{$fields.date_sent_c.name}" value="{$date_value}" title=''  tabindex='0'    size="11" maxlength="10" >
<button type="button" id="{$fields.date_sent_c.name}_trigger" class="btn btn-danger" onclick="return false;"><span class="suitepicon suitepicon-module-calendar" alt="{$APP.LBL_ENTER_DATE}"></span></button>
</span>
<script type="text/javascript">
Calendar.setup ({ldelim}
inputField : "{$fields.date_sent_c.name}",
form : "EditView",
ifFormat : "{$CALENDAR_FORMAT}",
daFormat : "{$CALENDAR_FORMAT}",
button : "{$fields.date_sent_c.name}_trigger",
singleClick : true,
dateStr : "{$date_value}",
startWeekday: {$CALENDAR_FDOW|default:'0'},
step : 1,
...
addToValidate('EditView', 'date_sent_c', 'date', false,'{/literal}{sugar_translate label='LBL_DATE_SENT' module='Leads' for_js=true}{literal}' );
...
addToValidate('EditView', 'gfr_events_leads_1_name', 'relate', false,'{/literal}{sugar_translate label='LBL_GFR_EVENTS_LEADS_1_FROM_GFR_EVENTS_TITLE' module='Leads' for_js=true}{literal}' );
addToValidate('EditView', 'captured_sent_date_c', 'bool', false,'{/literal}{sugar_translate label='LBL_CAPTURED_SENT_DATE' module='Leads' for_js=true}{literal}' );
addToValidate('EditView', 'contact_form_c', 'varchar', false,'{/literal}{sugar_translate label='LBL_CONTACT_FORM' module='Leads' for_js=true}{literal}' );
addToValidate('EditView', 'event_name_c', 'varchar', false,'{/literal}{sugar_translate label='LBL_EVENT_NAME' module='Leads' for_js=true}{literal}' );
...
addToValidate('EditView', 'event_type_c', 'enum', false,'{/literal}{sugar_translate label='LBL_EVENT_TYPE' module='Leads' for_js=true}{literal}' );
addToValidate('EditView', 'charity_name_c', 'varchar', false,'{/literal}{sugar_translate label='LBL_CHARITY_NAME' module='Leads' for_js=true}{literal}' );
addToValidate('EditView', 'event_date_c', 'date', false,'{/literal}{sugar_translate label='LBL_EVENT_DATE' module='Leads' for_js=true}{literal}' );
